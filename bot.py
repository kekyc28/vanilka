...
def get_access_decision_kb(user_id, access_type, nick, about, reason):
    builder = InlineKeyboardBuilder()
    # Кодируем данные для передачи
    data_str = f"{nick}|{about}|{reason}"
...
def get_user(user):
    # Поддержка разных форматов ID (DC2 и другие)
    if user.username:
...
async def complaint_media(msg: types.Message, state: FSMContext):
    ...
    if msg.text == "✅ Отправить":
        ...
        complaints_store[ticket] = {"user_id": msg.from_user.id, "text": text}
...
async def question_text(msg: types.Message, state: FSMContext):
    ...
    questions_store[ticket] = {"user_id": msg.from_user.id, "text": text}
...
async def access_reason(msg: types.Message, state: FSMContext):
    ...
    if access_type == "paid":
        ...
        await bot.send_message(ADMIN_ID, f"📨 Заявка на проходку\n👤 Ник: {nick}\n📝 О себе: {about}\n💭 Причина: {reason}\n🎟️ Бесплатная\n👤 Отправитель: {get_user(msg.from_user)}", reply_markup=get_access_decision_kb(msg.from_user.id, "free", nick, about, reason))
