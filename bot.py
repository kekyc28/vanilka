...
async def access_reason(msg: types.Message, state: FSMContext):
    ...
    if access_type == "paid":
        ...
        await bot.send_message(ADMIN_ID, admin_text, reply_markup=get_access_decision_kb(msg.from_user.id, "paid"))
    else:
        # Бесплатная проходка
...
async def process_screenshot(msg: types.Message, state: FSMContext):
    ...
    # Отправляем в канал уведомление с скриншотом
    channel_text = f"✅ Новая оплата\n📦 {product_name}\n👤 {nick}\n💰 {amount}₽"
    await bot.send_photo(CHANNEL_ID, msg.photo[-1].file_id, caption=channel_text)
    
    # Для платной проходки отправляем админу дополнительное уведомление с кнопками
