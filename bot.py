...
async def announcement_send(msg: types.Message, state: FSMContext):
    ...
    if TOPIC_ANNOUNCEMENTS:
        await send_to_channel(CHANNEL_ID, group_text, topic_id=TOPIC_ANNOUNCEMENTS)
    else:
        await send_to_channel(CHANNEL_ID, group_text)
...
async def complaint_media(msg: types.Message, state: FSMContext):
    ...
    if msg.photo:
        ...
        await msg.answer(f"📸 Фото добавлено. Всего: {len(media)}/10.\nОтправьте ещё или нажмите «✅ Отправить».")
    elif msg.video:
        media.append({'type': 'video', 'id': msg.video.file_id})
        await msg.answer(f"🎥 Видео добавлено. Всего: {len(media)}/10.\nОтправьте ещё или нажмите «✅ Отправить».")
    else:
...
async def question_text(msg: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    text = f"❓ Новый вопрос\n\n👤 Игрок: {data['nick']}\n💬 Вопрос: {msg.text}"
    await send_to_channel(CHANNEL_ID, text, topic_id=TOPIC_QUESTIONS)
