...
PRIVILEGES = [
    ...
    {"name": "DELUXE", "price": 500, "description": "все привилегии PREMIUM, /ec, 10 домов", "emoji": "👑"},
    {"name": "LEGEND", "price": 1000, "description": "все привилегии DELUXE, эффект легенды", "emoji": "💎"},
    {"name": "ULTRA", "price": 2000, "description": "все привилегии LEGEND, /nick, /speed", "emoji": "⚡"},
    {"name": "TITAN", "price": 3500, "description": "все привилегии ULTRA, команды Титана", "emoji": "🔱"},
    {"name": "GOD", "price": 5000, "description": "все привилегии TITAN, свой цвет в чате", "emoji": "👾"}
]
...
def get_access_type_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="🎟️ Бесплатная", callback_data="access_type_free")
    builder.button(text="💎 Платная (300₽)", callback_data="access_type_paid")
    builder.button(text="⬅️ Назад", callback_data="main_menu")
    builder.adjust(1)
    return builder.as_markup()
...
def get_access_decision_keyboard(user_id: int, access_type: str):
    builder = InlineKeyboardBuilder()
...
async def access_type_free(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🎟️ Бесплатная проходка\n\nВведите свой игровой ник:")
...
async def access_type_paid(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AccessStates.waiting_for_nick)
    await callback.message.edit_text("💎 Платная проходка (300₽)\n\nВведите свой игровой ник:")
    await callback.answer()
...
async def access_nick(message: types.Message, state: FSMContext):
    ...
    await message.answer("💭 Почему вы хотите играть именно у нас?", reply_markup=get_cancel_keyboard())
...
async def access_reason(message: types.Message, state: FSMContext):
    ...
    if access_type == "paid":
        admin_text += f"💎 Тип: Платная (300₽) — ожидает оплаты\n"
    else:
...
async def cancel_payment(callback: types.CallbackQuery):
    try:
        ...
        type_names = {
            "vanilla": "Пополнение Ванилек",
            "priv": "Покупка привилегии",
            "support": "Пожертвование",
            "paid_access": "Платная проходка"
        }
        type_name = type_names.get(operation_type, "Операция")
        ...
        # Отправляем подтверждение игроку
        await callback.message.answer(f"❌ {type_name} отменена.\n\nВы можете начать заново в любой момент.", reply_markup=get_main_keyboard())
        
        # Уведомляем админа
        await bot.send_message(
            ADMIN_ID,
            f"❌ ИГРОК ОТМЕНИЛ {type_name.upper()}\n👤 {get_user_identifier(callback.from_user)}\n🆔 {operation_id}"
        )
        
        await callback.answer(f"{type_name} отменена")
    except Exception as e:
...
async def complaint_proof(message: types.Message, state: FSMContext):
    ...
    if message.text == "✅ Завершить и отправить":
        ...
        for p in proofs:
            if p['type'] == 'photo':
                await bot.send_photo(CHANNEL_ID, p['id'], caption=f"📸 Доказательство от {data['nick']}")
            elif p['type'] == 'video':
                await bot.send_video(CHANNEL_ID, p['id'], caption=f"🎥 Доказательство от {data['nick']}")
...
async def support_nick(message: types.Message, state: FSMContext):
    ...
    await message.answer("💰 Введите сумму (от 10 до 100 000 рублей):", reply_markup=get_cancel_keyboard())
...
async def support_amount(message: types.Message, state: FSMContext):
    ...
    if amount < 10 or amount > 100000:
        await message.answer("❌ Сумма должна быть от 10 до 100 000 рублей")
        return
    data = await state.get_data()
    operation_id = f"{message.from_user.id}_{int(time.time())}"
...
async def vanilla_amount(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action == "custom":
        ...
        await callback.message.edit_text("🍦 Введите сумму (от 10 до 100 000 рублей):")
        await callback.answer()
        return
    amount = int(action)
