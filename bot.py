...
from aiogram.fsm.state import Simport asyncio
import logging
import os
import time
import re
...
def encode_callback_data(data: str) -> str:
    """Кодирует строку для безопасного использования в callback_data"""
    return data.replace(" ", "_").replace("🍃", "").replace("⭐", "").replace("👑", "").replace("💎", "").replace("⚡", "").replace("🔱", "").replace("👾", "")
...
def get_privilege_kb():
    builder = InlineKeyboardBuilder()
    for p in PRIVILEGES:
        safe_name = encode_callback_data(p['name'])
        builder.button(text=f"{p['emoji']} {p['name']} - {p['price']}₽", callback_data=f"priv_{safe_name}")
...
def get_payment_kb(operation_id, operation_type, details):
    builder = InlineKeyboardBuilder()
    safe_details = encode_callback_data(details)
    builder.button(text="✅ Я оплатил", callback_data=f"pay_{operation_type}_{operation_id}_{safe_details}")
...
def clean_product_name(name):
    ...
    cleaned = re.sub(r'[^a-zA-Zа-яА-Я0-9\s]', '', cleaned)
    return cleaned
...
@dp.message()
async def handle_all_messages(msg: types.Message, state: FSMContext):
    ...
    logging.info(f"Сообщение в состоянии {current_state}: {msg.text[:50]}...")
    
    # ========== ЖАЛОБЫ ==========
    if current_state == ComplaintStates.waiting_nick:
    ...
    elif current_state == ComplaintStates.waiting_offender:
    ...
    elif current_state == ComplaintStates.waiting_desc:
        ...
        if msg.text == "✅ Отправить":
            ...
            await msg.answer("✅ Жалоба отправлена! Администрация рассмотрит её в ближайшее время.", reply_markup=main_kb)
            await state.clear()
    ...
    # ========== ПРОХОДКА ==========
    elif current_state == AccessStates.waiting_nick:
        await state.update_data(nick=msg.text)
        await state.set_state(AccessStates.waiting_reason)
        await msg.answer(f"✅ Ник принят: {msg.text}\n\n💭 Почему вы хотите играть именно у нас?", reply_markup=cancel_kb)
    
    elif current_state == AccessStates.waiting_reason:
        data = await state.get_data()
        typ = data.get('typ')
        nick = data.get('nick')
        reason = msg.text
        ...
        if typ == "paid":
            ...
            await msg.answer("✅ Заявка отправлена! Администрация рассмотрит её и свяжется с вами.", reply_markup=main_kb)
            await bot.send_message(ADMIN_ID, f"📨 Заявка на проходку\n\n👤 Ник: {nick}\n💭 Причина: {reason}\n🎟️ Бесплатная\n👤 {get_user(msg.from_user)}", reply_markup=get_access_decision_kb(msg.from_user.id, "free"))
        await state.clear()
    
    # ========== ПОКУПКА ВАНИЛЕК ==========
    elif current_state == VanillaDonateStates.waiting_amount:
        ...
        await state.set_state(VanillaDonateStates.waiting_nick)
        await msg.answer(f"🍦 Сумма: {amount}₽\n\nВведите игровой ник:", reply_markup=cancel_kb)
    
    elif current_state == VanillaDonateStates.waiting_nick:
        data = await state.get_data()
    ...
    # ========== ПОКУПКА ПРИВИЛЕГИЙ ==========
    elif current_state == PrivilegeStates.waiting_nick:
        data = await state.get_data()
    ...
    # ========== ПОДДЕРЖКА ==========
    elif current_state == SupportStates.waiting_amount:
        if not msg.text.isdigit():
            await msg.answer("❌ Введите число!")
            return
        amount = int(msg.text)
        if amount < 10 or amount > 100000:
            ...
            return
        await state.update_data(amount=amount)
        await state.set_state(SupportStates.waiting_nick)
        await msg.answer(f"💝 Сумма: {amount}₽\n\nВведите игровой ник:", reply_markup=cancel_kb)
    
    elif current_state == SupportStates.waiting_nick:
        data = await state.get_data()
        amount = data.get('amount')
        nick = msg.text
        op_id = f"{msg.from_user.id}_{int(time.time())}"
    ...
    # ========== ОТВЕТЫ АДМИНА ==========
    elif current_state == ReplyState.waiting_text:
        data = await state.get_data()
...
async def shop_vanilla(call: types.CallbackQuery, state: FSMContext):
...
async def vanilla_buy(call: types.CallbackQuery, state: FSMContext):
    ...
    if action == "custom":
        await state.set_state(VanillaDonateStates.waiting_amount)
...
async def priv_buy(call: types.CallbackQuery, state: FSMContext):
    # Декодируем название привилегии
    encoded_name = call.data.split("_")[1]
    # Восстанавлием оригинальное название
    name_map = {p['name'].replace(" ", "_"): p['name'] for p in PRIVILEGES}
    name = name_map.get(encoded_name, encoded_name)
    priv = next((p for p in PRIVILEGES if p['name'] == name), None)
    if not priv:
        await call.answer("❌ Ошибка.")
        return
    await state.update_data(priv_name=priv['name'], priv_price=priv['price'])
    await state.set_state(PrivilegeStates.waiting_nick)
...
async def shop_support(call: types.CallbackQuery, state: FSMContext):
    await state.set_state(SupportStates.waiting_amount)
...
async def access_free(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AccessStates.waiting_nick)
...
async def question_start(msg: types.Message, state: FSMContext):
    await state.set_state(QuestionStates.waiting_nick)
...
async def payment_confirm(call: types.CallbackQuery):
    try:
        ...
        typ = parts[1]
        op_id = parts[2]
        # Декодируем details из оставшихся частей
...
async def reply_start(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("✏️ Введите ответ для игрока (можно длинное сообщение):")
    await call.answer()
...
if __name__ == "__main__":
    asyncio.run(main())tate, StatesGroup
from aiogram.fsm.storage.memory import MemoryStorage
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, ReplyKeyboardMarkup, KeyboardButton
from aiogram.utils.keyboard import InlineKeyboardBuilder
