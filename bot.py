...
pending_tickets = {}  # {ticket_id: {"user_id": int, "user_nick": str, "chat_id": int, "message_id": int, "type": str}}
users_db = set()
...
def get_reply_kb(ticket_type, ticket_id, user_id):
    builder = InlineKeyboardBuilder()
    builder.button(text="✅ Ответить", callback_data=f"reply_{ticket_type}_{ticket_id}_{user_id}")
    builder.button(text="❌ Закрыть", callback_data=f"close_{ticket_type}_{ticket_id}_{user_id}")
    builder.adjust(2)
    return builder.as_markup()
...
async def complaint_media(msg: types.Message, state: FSMContext):
    ...
    if msg.text == "✅ Отправить":
        ...
        # Текст жалобы для группы
        text = f"⚠️ НОВАЯ ЖАЛОБА\n\n👤 Заявитель: {user_nick}\n🤬 Нарушитель: {data['offender']}\n📝 Описание: {data['desc']}"
        
        # Отправляем текст в группу
        msg_in_group = await send_to_channel(CHANNEL_ID, text, topic_id=TOPIC_COMPLAINTS)
        
        # Отправляем медиа группой (одним сообщением)
        if media:
            media_group = []
            for m in media:
                if m['type'] == 'photo':
                    media_group.append(types.InputMediaPhoto(media=m['id'], caption=f"📸 Доказательства от {user_nick}" if len(media_group) == 0 else ""))
                elif m['type'] == 'video':
                    media_group.append(types.InputMediaVideo(media=m['id'], caption=f"🎥 Доказательства от {user_nick}" if len(media_group) == 0 else ""))
            if media_group:
                await send_to_channel(CHANNEL_ID, media_group=media_group, topic_id=TOPIC_COMPLAINTS)
        
        # Сохраняем информацию о тикете для последующего редактирования
        pending_tickets[ticket_id] = {
            "user_id": msg.from_user.id,
            "user_nick": user_nick,
            "chat_id": CHANNEL_ID,
            "message_id": msg_in_group.message_id,
            "topic_id": TOPIC_COMPLAINTS,
            "type": "complaint"
        }
        
        # Отправляем админу
        admin_text = f"📨 НОВАЯ ЖАЛОБА\n\n👤 Заявитель: {user_nick}\n🤬 Нарушитель: {data['offender']}\n📝 Описание: {data['desc']}\n👤 Отправитель: {get_user(msg.from_user)}"
        admin_msg = await bot.send_message(ADMIN_ID, admin_text, reply_markup=get_reply_kb("complaint", ticket_id, msg.from_user.id))
        
        # Отправляем админу медиа группой
        if media:
            admin_media_group = []
            for m in media:
                if m['type'] == 'photo':
                    admin_media_group.append(types.InputMediaPhoto(media=m['id'], caption=f"📸 Доказательства от {user_nick}" if len(admin_media_group) == 0 else ""))
                elif m['type'] == 'video':
                    admin_media_group.append(types.InputMediaVideo(media=m['id'], caption=f"🎥 Доказательства от {user_nick}" if len(admin_media_group) == 0 else ""))
            if admin_media_group:
                await bot.send_media_group(ADMIN_ID, admin_media_group)
        
        pending_tickets[ticket_id]["admin_msg_id"] = admin_msg.message_id
...
async def question_text(msg: types.Message, state: FSMContext):
    ...
    text = f"❓ НОВЫЙ ВОПРОС\n\n👤 Игрок: {user_nick}\n💬 Вопрос: {msg.text}"
    msg_in_group = await send_to_channel(CHANNEL_ID, text, topic_id=TOPIC_QUESTIONS)
    ...
    pending_tickets[ticket_id] = {
        ...
        "chat_id": CHANNEL_ID,
        "message_id": msg_in_group.message_id,
        "topic_id": TOPIC_QUESTIONS,
        "type": "question"
    }
    
    admin_text = f"📨 НОВЫЙ ВОПРОС\n\n👤 Игрок: {user_nick}\n💬 Вопрос: {msg.text}\n👤 Отправитель: {get_user(msg.from_user)}"
    admin_msg = await bot.send_message(ADMIN_ID, admin_text, reply_markup=get_reply_kb("question", ticket_id, msg.from_user.id))
    pending_tickets[ticket_id]["admin_msg_id"] = admin_msg.message_id
...
async def reply_start(call: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(reply_user=user_id, reply_ticket_id=ticket_id, reply_ticket_type=ticket_type)
...
async def reply_send(msg: types.Message, state: FSMContext):
    ...
    ticket_info = pending_tickets.get(ticket_id)
    
    if ticket_type == "complaint":
        reply_text = f"📨 Ответ администратора на вашу жалобу\n\n{msg.text}\n\nС уважением, администрация сервера."
        channel_text = f"✅ Ответ администратора на жалобу игрока {ticket_info['user_nick'] if ticket_info else 'неизвестного'}\n\n{msg.text}"
    else:
        reply_text = f"📨 Ответ администратора на ваш вопрос\n\n{msg.text}\n\n💡 Если остались вопросы — напишите снова."
        channel_text = f"✅ Ответ администратора на вопрос игрока {ticket_info['user_nick'] if ticket_info else 'неизвестного'}\n\n{msg.text}"
    ...
    try:
        ...
        # Редактируем исходное сообщение в группе
        if ticket_info and ticket_info.get("message_id"):
            try:
                # Получаем исходное сообщение
                original_msg = await bot.forward_message(
                    chat_id=ticket_info["chat_id"],
                    from_chat_id=ticket_info["chat_id"],
                    message_id=ticket_info["message_id"]
                )
                # Редактируем его
        ...
        # Отправляем в канал
        await send_to_channel(CHANNEL_ID, channel_text, topic_id=TOPIC_REPLIES)
        
        # Обновляем сообщение у администратора
        if ticket_info and ticket_info.get("admin_msg_id"):
            await bot.edit_message_text(
                chat_id=ADMIN_ID,
                message_id=ticket_info["admin_msg_id"],
...
async def close_ticket(call: types.CallbackQuery):
    ...
    if ticket_type == "complaint":
        channel_text = f"✅ Жалоба игрока {ticket_info['user_nick'] if ticket_info else 'неизвестного'} закрыта администратором"
        user_msg = "✅ Ваша жалоба закрыта администратором.\n\nСпасибо за обращение!"
    else:
        channel_text = f"✅ Вопрос игрока {ticket_info['user_nick'] if ticket_info else 'неизвестного'} закрыт администратором"
